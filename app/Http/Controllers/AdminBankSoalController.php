...
class AdminBankSoalController extends Controller
{
    ...
    {
        try {
            // Ambil semua mapping mapel dari seluruh guru dengan ujian aktif
            ...
            $bankSoals = BankSoal::with('guru')->paginate(10);

            return view('admin.BankSoal.index', compact('mapingMapels', 'kelas', 'bankSoals'));
        } catch (\Exception $e) {
    ...
    public function store(Request $request)
    {
        try {
            ...
            // Ambil DataUjian yang aktif langsung dari tabel data_ujians
            $dataUjian = \App\Models\DataUjian::with('tahunPelajaran')
                ->where('status', true)
                ->first();
            ...
            $existingSoal = BankSoal::whereJsonContains('mata_pelajaran_id->mata_pelajaran_id', $mapelId)
            ...
            BankSoal::create([
                ...
                'guru_id' => $guruId,
                'data_ujian_id' => $dataUjian->id, // ✅ Tambahkan ini
                'mata_pelajaran_id' => json_encode($mapelKelasData), // Simpan sebagai JSON
                'file_soal' => $filePath,
                'status' => true, // Otomatis aktif
            ]);

            return redirect()->route('admin.bank-soal.index')->with('success', 'Bank Soal berhasil ditambahkan!');
        } catch (\Exception $e) {
    ...
    public function simpanValidasiSoal(Request $request)
    {
        ...
        $bankSoal = BankSoal::findOrFail($request->bank_soals_id);
        $guru_id = $bankSoal->guru_id;
