...
@section('content')
                ...
                <tbody>
                    @forelse ($bankSoals as $index => $soal)
                        ...
                        <tr>
                            ...
                            <td>{{ $soal->guru->Nama ?? 'Tidak diketahui' }}</td>
    ...
    <div class="modal fade" id="addBankSoalModal" data-bs-backdrop="static" tabindex="-1"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form id="bankSoalForm" action="{{ route('admin.bank-soal.store') }}" method="POST"
                                ...
                                @foreach ($mapingMapels as $maping)
                                    @foreach ($maping->mapel_kelas_list as $data)
                                        ...
                                        <option value="{{ $data['mata_pelajaran_id'] }}"
                                            ...
                                            {{ $data['mapel'] }} - {{ $maping->guru->Nama ?? 'Guru Tidak Diketahui' }} - {{ $kelasText }}
                                        </option>
                                    @endforeach
                                @endforeach
                            </select>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            let guruIdHidden = document.getElementById("guruIdHidden");
            ...
            mataPelajaranSelect.addEventListener("change", function() {
                ...
                let guruId = selectedOption.getAttribute("data-guru");

                // Set nilai ke input hidden
                guruIdHidden.value = guruId;
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            document.querySelectorAll(".open-preview-modal").forEach(function(element) {
                element.addEventListener("click", function() {
                    ...
                    fetch(`/admin/bank-soal/preview/${soalId}`)
                        .then(response => response.json())
                        .then(data => {
        ...
        `;
                                ...
                                validasiCheckbox.onchange = function() {
                                    ...
                                    fetch('/admin/bank-soal/validasi', {
                                            method: 'POST',
                                            headers: {
